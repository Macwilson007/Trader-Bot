"""
QUICK PARAMETER TEST - Single session hours, fast
"""
...
def test_params(df, adx_min, atr_thresh, sl_mult, tp2_r, hours=[8]):
    initial = 7157.31
    balance = initial
    wins, losses = 0, 0
    ...
    closed = wins + losses
    return {
        'adx': adx_min, 'atr': atr_thresh, 'sl': sl_mult, 'tp': tp2_r,
        'wins': wins, 'losses': losses, 'wr': wins/closed*100 if closed else 0,
        'pf': (wins * tp2_r) / losses if losses else 99,
        'pnl': balance - initial, 'bal': balance
    }

print("Loading data...")
h1 = mt5.copy_rates_range("XAUUSDc", mt5.TIMEFRAME_H1, datetime.now() - timedelta(days=180), datetime.now())
...
if 'time' not in df.columns:
    df = df.rename(columns={0: 'time', 1: 'open', 2: 'low', 3: 'high', 4: 'close', 5: 'tick_volume', 6: 'spread', 7: 'real_volume'})
df['time'] = pd.to_datetime(df['time'], unit='s')
print(f"Data: {len(df)} bars\n")

results = []
configs = [
    (25, 2.5, 1.5, 2.5),  # original
    (30, 2.0, 2.0, 2.0),
    (35, 1.5, 2.5, 2.0),
    (40, 1.5, 3.0, 2.0),
    (35, 1.5, 2.0, 3.0),
...
for adx, atr, sl, tp in configs:
    r = test_params(df, adx, atr, sl, tp, hours=[8, 13])
    results.append(r)
    print(f"ADX>{adx} ATRx<{atr} SL:{sl}x TP:{tp}x | WR:{r['wr']:.0f}% PF:{r['pf']:.1f} | ${r['pnl']:+,.0f}")

print("\n" + "="*60)
print("SORTED BY P&L")
print("="*60)
for r in sorted(results, key=lambda x: x['pnl'], reverse=True):
    print(f"ADX>{r['adx']} ATRx<{r['atr']} SL:{r['sl']}x TP:{r['tp']}x | WR:{r['wr']:.0f}% PF:{r['pf']:.1f} W:{r['wins']} L:{r['losses']} | ${r['pnl']:+,.0f}")
