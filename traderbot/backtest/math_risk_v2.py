"""
MATHEMATICAL RISK ENGINE v2 - Adjusted BOS
"""
...
class MathRiskEngineV2:
    ...
    def check_bos_simple(self, df, idx, direction):
        if idx < 10:
            return False
        
        lookback = df.iloc[idx-9:idx+1]
    ...
    def run_backtest(self):
        print("="*80)
        print("MATHEMATICAL RISK ENGINE v2 - Adjusted BOS")
        print("="*80)
        ...
        stats = {"regime_ok": 0, "blocked": 0, "bos_pass": 0, "total": 0}
        ...
        for i in range(100, len(df) - 12):
            ...
            if not self.check_bos_simple(df, i, direction):
                continue
            ...
            if tp2_hit:
                ...
                if direction == "LONG":
                    pnl = (final_price - entry) / self.get_pip_size() * 3.5 * lot_size
                else:
                    pnl = (entry - final_price) / self.get_pip_size() * 3.5 * lot_size
    ...
    def print_results(self):
        ...
        for t in self.trades:
            print(f"{t['time'].strftime('%Y-%m-%d %H:%M')} | {t['direction']:5} | "
                  f"ATR:{t['atr']:.2f} ADX:{t['adx']:.1f} | Lot:{t['lot']:.3f} | "
                  f"{t['result']} | ${t['pnl']:+,.2f}")
